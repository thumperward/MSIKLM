...
I tried to keep the external dependencies to a minimum level, however there are
some unavoidable ones. These are:
...
Besides there are no others, no Qt, no Java, not even a C++ compiler is
required. To install the program on any Debian-based Linux distribution (for
instance any ubuntu-based one), there is an installation script `install.sh`
which can be run by opening the respective folder in a terminal and typing:
...
as well which most certainly will work on most Debian-based distributions. This
script will do the following steps, if you do not want to use the installation
script for some reason, you can use the manual commands instead:
...
Whenever MSIKLM is used, it should always be run as root because otherwise, the
communication with the keyboard is not possible, hence always use the sudo
prefix (only 'msiklm help' will work as non-root).
...
example 'sudo msiklm green' will set the color green using its rgb-values (i.e.
red=0, green=255, blue=0 or 0x00FF00 in hex code notation) while 'sudo msiklm
...
-   `sudo msiklm list`: lists all found hid devices, this might be helpful if
    your keyboard is not detected by MSIKLM

## Autostart

An important additional feature is the optional autostart functionality since
the keyboard will reset itself to its default color configuration whenever you
reboot it or resume from standby. Hence, it is really useful to automatically
reconfigure the keyboard to your configuration of choice. To do this, there is
an extra script called 'autostart.sh' that can do this for you. This script
registers MSIKLM to the udev service (more precisely it registers the keyboard
to the udev service which calls MSIKLM as soon as the keyboard is detected) by
...
Try if everything works by first rebooting your system and then try a standby
and wakeup. If everything works, we are done here. If not, please report an
issue. :-)
...
MSIKLM also comes with an uninstallation script uninstall.sh which will remove
the program file /usr/local/bin/msiklm as well as running
`./autostart --disable`, i.e. it disables the autostart. If you want to use it,
simply run:
...
-   Specify that the USB library needed is `libhidapi-libusb0`.
-   Properly document how to run as non-root, by adding the following to
    `/etc/udev/rules.d/89-msiklm.rules`:

        ```cfg
        SUBSYSTEM=="usb", ATTR{idVendor}=="1770", ATTR{idProduct}=="ff00", MODE="666"
        ```

    and then running `sudo udevadm trigger`.
